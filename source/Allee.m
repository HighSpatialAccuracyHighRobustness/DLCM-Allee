...
Drate = 0.0047; % (the simulation is rather sensitive to this parameter!)
DrateRandom = 0.0047;
A = -3;
tau = 7.5;
lam=0.01;
...
centerPoint = find(max(abs(P),[],1) <= 1.1*h);
seeding = randi(length(centerPoint),1,4);
U = fsparse(centerPoint(seeding)',1,3,[Nvoxels^3 1]);
% (format: U = {0,1,2} for {empty,one cell,two cells})

% simulation interval
Tend = 840;
% solution recorded at this grid:
tspan = linspace(0,Tend,211);
...
while tt <= tspan(end)
  ...
  moves = Drate*full(grad)+DrateRandom;
  
  % (2) cell proliferation festured by Allee Effect
   prolif = lam*U(pdof).*(1-(A+tau)./(U(pdof)+tau));
  ...
  elseif ix_ <= numel(moveb)+numel(moves)
    ...
    jx_ = find(N(ix,Adof));
    jx_ = jx_(U(Adof(jx_)) < 3); % only allow moves to less populated voxels
